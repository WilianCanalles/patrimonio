...
function conectar()
{
    ...
    try {
        ...
        $query = "select email, usuario, b.`codigo`, b.`empresa` from `tb_user` INNER JOIN `tb_empresa` AS b ON `tb_user`.`emp_Principal` = b.`codigo` where (email = :email AND senha = md5(:senha)) OR (usuario = :email AND senha = md5(:senha))";
